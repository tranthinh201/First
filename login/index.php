<?php
	...
	$password = $_POST['password'] ?? '';
	    ...
	    if (isset($_POST['login'])) {
	    	if(empty($username)) {
	    		echo '<script language="javascript">alert("Tài khoản không được bỏ trống");</script>';
	    	}
	    	else if (empty($password)) {
	    		echo '<script language="javascript">alert("Mật khẩu không được bỏ trống");</script>';
	    	}
...
<body>
	...
	<div class="container">
		<div class="grid">
			<h1 class="content text--center">Đăng nhập</h1>
			<form method="POST" class="form login">
				<div class="form__field">
					<label for="login__username"><svg class="icon">
							<use xlink:href="#icon-user"></use>
						</svg><span class="hidden">Tài khoản</span></label>
					<input id="login__username" type="text" name="username" class="form__input" placeholder="Tài khoản" >
				</div>
			...
			<p class="text--center text-g">Bạn chưa có tài khoản? <a href="../register" class="change-ps">Đăng ký ngay</a> <svg class="icon icon-red">
